...
ENVIRONMENT VARIABLES
  ...
  PLAN                  See the deployment before executing it.
                        If you want to preview a production deployment, set
                        PRODUCTION to true as well.
                        (Default: true)

  PRODUCTION            Deploys the website to the CloudFront CDN and issues
...
for stage in tf-init tf-plan tf-apply export-slides copy-slides
do
  if [ "$stage" == "tf-apply" ] && [ "$PRODUCTION" == "true" ]
  then
    >&2 echo "INFO: Deploying to production."
    stage="tf-apply-prod"
  fi
  if [ "$stage" == "tf-plan" ] && [ "$PRODUCTION" == "true" ]
  then
    >&2 echo "INFO: Generating preview of your production deployment."
    stage="tf-plan-prod"
  fi
  ...
  SLIDES_DIR=./$talk_name \
    ...
    -e AWS_REGION \
    "$stage"
  if [ "$PLAN" == "true" ] && $(echo "$stage" | grep -Eq "tf-plan|tf-plan-prod")
  then
    >&2 echo "INFO: Stopping since we only wanted a plan."
    break
  fi
done
